...
class Atm:
    ...
    def __init__(self, bank):
        ...
        self.DISPLAY_TRANSACTIONS = 4
        self.EXIT_ACCOUNT_OPTION = 5
    ...
    def manageAccount(self, account):
        ...
        while True:
            ...
            elif selAcctMenuOpt == self.DISPLAY_TRANSACTIONS:
                self.onDisplayTransactions(account)
    ...
    def onDisplayTransactions(self, account):
        """Display the properties of the given account and the list of transactions
        Arguments:
            account - the account whose information is to be displayed
        """
        print('\n========= TRANSACTION LIST ==============\n')
        
        #display the account type
        print('Checquing Account' if isinstance(account, ChecquingAccount) 
              else 'Savings Account' if isinstance(account, SavingsAccount) else 'General Account')
        
        #display the account number
        print('Account No: {0}'.format(account.getAccountNumber()))

        #display the client name
        print('Client: {0}\n'.format(account.getAcctHolderName()))

        #display the list of transactions performed on this account
        for trans in account.getTransactionList():
            print(trans)